  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Parceiros / Estabelecimentos</h1>
          <p className="text-sm text-gray-500">{partners.length} parceiro(s) cadastrado(s)</p>
        </div>
        <button
          onClick={openNew}
      ...
      {partners.length === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-medium text-gray-700 mb-1">Nenhum parceiro cadastrado</h3>
          <p className="text-sm text-gray-500">Cadastre fornecedores, lojas e prestadores de servico</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {partners.map(p => (
            <div key={p.id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3 hover:shadow-sm transition-shadow">
              ...
              <div className="flex gap-1">
                <button onClick={() => openEdit(p.id)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600">
                  <Edit3 size={14} />
                </button>
                {confirmDelete === p.id ? (
                  <div className="flex gap-1">
                    <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100">
                      <Check size={14} />
                    </button>
                  ...
                  <button onClick={() => setConfirmDelete(p.id)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-red-500">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center" onClick={() => setShowForm(false)}>
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl p-5 sm:p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">{editId ? 'Editar Parceiro' : 'Novo Parceiro'}</h2>
